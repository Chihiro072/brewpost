...
export const RedesignedMainLayout: React.FC<RedesignedMainLayoutProps> = ({
  ...
  // Periodically refetch nodes to sync with backend (auto-published status updates)
  useEffect(() => {
    if (nodes.length === 0) return // Only refetch if we have nodes loaded

    const interval = setInterval(async () => {
      try {
        const freshNodes = await fetchNodes()
        setNodes(prevNodes => {
          // Merge fresh data with local changes, preserving local edits
          return prevNodes.map(localNode => {
            const fresh = freshNodes.find(n => n.id === localNode.id)
            if (!fresh) return localNode
            // Update status and postedAt from backend, but preserve other local edits
            return {
              ...localNode,
              status: fresh.status,
        ...
        console.log('[RedesignedMainLayout] Refetched nodes, updated statuses')
      } catch (error) {
        console.warn('[RedesignedMainLayout] Failed to refetch nodes:', error)
      }
  ...
  const handleConfirmSchedule = async (scheduledNodes?: ContentNode[]) => {
    try {
      ...
      setNodes(prev =>
        prev.map(node => {
          const scheduledNode = nodesToSchedule?.find(sn => sn.id === node.id)
  ...
  const savePlannerMutation = useMutation({
    ...
    mutationFn: async ({
      ...
      plannerData: { title?: string; nodes: ContentNode[] }
      setClean: () => void
    }) => {
      const title =
        plannerData.title ?? `Draft ${new Date().toLocaleDateString()}`
      if (!currentPlanId) {
        const id = await plannerService.createFromNodes(
          title,
          plannerData.nodes
        )
        setCurrentPlanId(id)
        ...
        const replaced = existing.map((p: any, idx: number) => {
          if (idx === 0 && p.id && String(p.id).startsWith('temp-')) {
            return {
              ...
              title: usedTitle,
              createdAt: new Date().toISOString(),
              postCount: variables?.plannerData?.nodes?.length ?? 0
            }
          }
          return p
        })
