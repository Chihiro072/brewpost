...
export const QuickSettingsModal: React.FC<QuickSettingsModalProps> = ({ open, onOpenChange }) => {
  ...
  const handleNotificationsToggle = (checked: boolean) => {
    ...
    try {
      ...
      toast.success(checked ? t("settings.notifications") + " On" : t("settings.notifications") + " Off");
    } catch {}
  };
      ...
      <DialogContent className="max-w-md bg-[rgba(3,34,33,0.92)] backdrop-blur-xl border border-[#03624C]/50 shadow-[0_0_30px_rgba(44,194,149,0.25)] rounded-2xl data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95">
        ...
        <div className="space-y-5">
          ...
          <div className="flex justify-end pt-2">
            <Button variant="secondary" className="bg-[#03624C]/30 border border-[#2CC295]/40" onClick={() => onOpenChange(false)}>
              {t("common.close")}
            </Button>
          </div>
        </div>
