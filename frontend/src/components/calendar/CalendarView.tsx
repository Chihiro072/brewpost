...
export const CalendarView: React.FC<CalendarViewProps> = ({
    ...
    ;(async () => {
      try {
        ...
        if (!storedUserId) {
          ...
          console.info(
            'No userId available locally; server may require session auth.'
          )
        }
        ...
        try {
          ...
          console.warn(
            'Failed to fetch nodes from nodes table, falling back to schedules:',
            err
          )
          try {
            ...
            if (data.ok && Array.isArray(data.schedules) && mounted) {
              ...
              console.log(
                'First schedule title field:',
                data.schedules[0]?.title
              )
              console.log(
                'First schedule raw data:',
                JSON.stringify(data.schedules[0], null, 2)
              )
              ...
              if (mounted) setFetchedNodes(parsed)
              return
            }

            // If not ok, inspect error and surface actionable messages
            console.error('Unexpected schedule list response:', data)

            // handle missing_userid by prompting user to sign in (redirect to backend login)
            if (data.error === 'missing_userid') {
              const goLogin = confirm(
                'You must sign in to view your schedules. Redirect to the sign-in page now?'
              )
              if (goLogin) {
            ...
            if (data.error === 'unexpected_function_response') {
              console.warn(
                'Per-user schedules fetch returned unexpected shape detail:',
                data.detail
              )
              alert(
                'Failed to load schedules: per-user function returned unexpected data. Check console for "schedules/list" logs and paste the "detail" here for help.'
              )
              if (mounted) setFetchedNodes([])
              return
            }
            ...
            if (
              /dynamodb:Scan/i.test(detail) ||
              /DynamoDBScanAuthorizationError/i.test(data.error) ||
              /DynamoDB authorization error/i.test(data.error)
            ) {
              alert(
                'Failed to load schedules: server is missing DynamoDB read (Scan) permission or per-user fetch is not configured. Attach read permissions or configure the per-user Lambda. See aws/schedules-scan-policy.json for an example policy.'
              )
              if (mounted) setFetchedNodes([])
              return
            }
            ...
            if (
              data.error === 'unexpected_function_response' ||
              data.error === 'function_url_error' ||
              data.error === 'missing_userid'
            ) {
              console.warn('Per-user schedules fetch returned:', data)
              alert(
                'Failed to fetch per-user schedules. Check console logs and ensure your userId is provided (localStorage.userId) or session is active.'
              )
              if (mounted) setFetchedNodes([])
              return
            }
            ...
            console.error('Failed to fetch schedules fallback:', fallbackErr)
            if (mounted) setFetchedNodes([])
            return
          }
        }
  ...
  const nodesToRender =
    scheduledNodes && scheduledNodes.length > 0 ? scheduledNodes : fetchedNodes
  ...
  nodesToRender.forEach(node => {
    if (node.scheduledDate) {
      ...
      scheduledContent[dateKey].push({
        ...
        title: node.title,
        type: node.type,
        time: node.scheduledDate.toLocaleTimeString([], {
          hour: '2-digit',
          minute: '2-digit'
        }),
  ...
  return (
    <div className='fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4'>
      <Card className='w-full max-w-4xl max-h-[90vh] overflow-auto bg-[#03201f] backdrop-blur-xl border-[#03624C]/60 text-white'>
        <div className='p-6'>
          {/* Header */}
          <div className='flex items-center justify-between mb-6'>
            <div className='flex items-center gap-3'>
              <CalendarIcon className='w-6 h-6 text-primary' />
              <h2 className='text-2xl font-semibold bg-gradient-to-r from-[#2CC295] via-[#00DF81] to-[#03624C] bg-clip-text text-transparent'>
                Content Calendar
              </h2>
            </div>
            <Button
          ...
          <div className='grid grid-cols-7 gap-1 mb-4'>
            ...
            {days.map((day, index) => {
              ...
              return (
                <Card
                  key={day}
                  className={`p-2 min-h-[100px] rounded-xl border border-[#0d4d43]/60 bg-[#051414] shadow transition-colors`}
                >
                  ...
                  <div className='space-y-1'>
                    {dayContent.slice(0, 2).map(content => {
                      const fullEvent = nodesToRender.find(
                        node => node.id === content.id
                      )
                      return (
                        <div
                          key={content.id}
