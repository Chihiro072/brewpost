...
export const getTemplateSettings = (): TemplateSettings | null => {
  try {
    ...
    console.error(
      "🔧 [getTemplateSettings] Error parsing template settings:",
      error
    );
...
export const enhanceImagePromptWithTemplate = (
  ...
  if (template.logoPreview && template.selectedPosition) {
    ...
    enhancedPrompt += ` Leave space in the ${
      positionMap[template.selectedPosition]
    } for a logo overlay.`;
  }
...
export const applyTemplateToImage = async (
  imageUrl: string
): Promise<string> => {
  console.log(
    "[templateUtils] Starting applyTemplateToImage with URL:",
      ...
      img.onload = () => {
        ...
        if (settings.logoPreview) {
          ...
          logoImg.onload = () => {
            ...
            console.log(
              "[templateUtils] Drawing logo at position:",
              logoX,
              logoY,
              "size:",
              logoWidth,
...
export const convertToProxyUrl = (imageUrl: string): string => {
  ...
  try {
    ...
    const host = url.host;
    const isBrewpostS3Host =
      /s3-brewpost\.s3(?:\.[a-z0-9-]+)?\.amazonaws\.com/i.test(host);
    const isBrewpostAssets =
      host.includes("brewpost-assets") || imageUrl.includes("brewpost-assets");
    if (isBrewpostS3Host || isBrewpostAssets) {
      ...
      const proxyUrl = `${API_BASE_URL}/api/assets/proxy/${s3Key}`;
      return proxyUrl;
    }
  } catch (error) {
...
const applyCompanyTextToCanvas = (
  ...
  // Set up text styling
  const fontSize = settings.textSize
    ? Math.max(16, (settings.textSize * canvas.width) / 1000)
  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const promo = components.find(
          (c) =>
            (c.category &&
              String(c.category).toLowerCase().includes("promotion")) ||
        ...
        if (promo) {
          ...
          if (promo.color && typeof promo.color === "string") {
            ...
            baseHex = rawColor.startsWith("#")
              ? expandShortHex(rawColor)
              : stringToDeterministicHex(String(rawColor));
          } else {
            const palette = [
              "#FFB86B",
              "#FF6B6B",
          ...
          if (
            maybePos &&
            typeof maybePos === "object" &&
            ("x" in (maybePos as object) || "y" in (maybePos as object))
          ) {
          ...
          centerX = clamp(
            centerX,
            padding + circleBadgeSize / 2,
            canvas.width - padding - circleBadgeSize / 2
          );
          centerY = clamp(
            centerY,
            padding + circleBadgeSize / 2,
          ...
          try {
            ...
            if (tpl) {
              const logoBox =
                tpl.logoPreview && tpl.selectedPosition
                  ? (() => {
                      const logoSize =
                      ...
                      switch (tpl.selectedPosition) {
                        case "top-left":
                          ...
                          break;
                        case "top-center":
                          lx = (canvas.width - logoSize) / 2;
                          ly = paddingLocal;
                          break;
                        case "top-right":
                          lx = canvas.width - logoSize - paddingLocal;
                          ly = paddingLocal;
                          break;
                        case "bottom-left":
                          lx = paddingLocal;
                          ly = canvas.height - logoSize - paddingLocal;
                          break;
                        case "bottom-center":
                          lx = (canvas.width - logoSize) / 2;
                          ly = canvas.height - logoSize - paddingLocal;
              ...
              const textBox = tpl.companyText
                ? (() => {
                    const fontSize =
                      (tpl.textSize || 24) * (canvas.width / 1000);
                    // estimate width as quarter of canvas or based on font size
                    const tw = Math.min(
                      canvas.width * 0.4,
                      fontSize * (tpl.companyText?.length || 8)
                    );
                    const paddingLocal = Math.max(20, canvas.width * 0.02);
                    let tx = paddingLocal,
                      ty = paddingLocal;
                    switch (tpl.selectedPosition) {
                      ...
                      case "top-center":
                        ...
                        break;
                      case "top-right":
                        tx = canvas.width - tw - paddingLocal;
                        ty = paddingLocal + fontSize + 4;
                        break;
                      case "bottom-left":
                        tx = paddingLocal;
                        ty = canvas.height - paddingLocal - fontSize;
                        break;
                      case "bottom-center":
                        tx = (canvas.width - tw) / 2;
                        ty = canvas.height - paddingLocal - fontSize;
          ...
          const g = ctx.createRadialGradient(
            centerX - circleBadgeSize * 0.12,
            centerY - circleBadgeSize * 0.18,
            circleBadgeSize * 0.05,
            centerX,
            centerY,
            circleBadgeSize / 1.1
          );
          ...
          g.addColorStop(
            1,
            hexToRgba(adjustColorLuminance(badgeHex, -0.06), 1)
          );
          ctx.save();
          ...
          ctx.beginPath();
          ctx.ellipse(
            centerX - circleBadgeSize * 0.16,
            centerY - circleBadgeSize * 0.22,
            circleBadgeSize * 0.28,
            circleBadgeSize * 0.18,
            -0.4,
            0,
            Math.PI * 2
          );
        ...
        try {
          ...
          console.warn(
            "Canvas tainted when applying components, returning original image"
          );
          resolve(imageUrl);
        }
      };
