...
export async function createNodeService(
  ...
  try {
    const requestData = {
      ...
      connections: Array.isArray(nodeData.connections)
  ...
  try {
    ...
    if (response.data) {
      return {
        ...
        connections: Array.isArray(response.data.connections)
          ? response.data.connections
          : [],
        position: { x: response.data.x || 0, y: response.data.y || 0 },
        postedAt: response.data.postedAt
          ? new Date(response.data.postedAt)
          : undefined,
        postedTo: response.data.postedTo,
        tweetId: response.data.tweetId,
        selectedImageUrl: response.data.selectedImageUrl,
      };
    }
    return null;
  } catch (error) {
    console.error("[nodeService] Error scheduling node:", error);
    throw error;
  }
}

export async function scheduleAllNodesService(
  projectId?: string
): Promise<number> {
  try {
    const url = projectId
    ...
    // Response includes a message like "X nodes scheduled successfully"
    // Parse the count from the message
    const count = parseInt(response.data.message?.match(/\d+/)?.[0] || "0");
    return count;
  } catch (error) {
    console.error("[nodeService] Error scheduling all nodes:", error);
    throw error;
  }
}
...
export const NodeAPI = {
  ...
  async createEdge(projectId: string, from: string, to: string) {
    try {
      ...
      const current = Array.isArray(source.connections)
        ? source.connections
        : [];
  ...
  subscribe(
    projectId: string,
    onEvent: (evt: {
      type: "created" | "updated" | "deleted" | "edge";
      payload: any;
    }) => void
  ) {
